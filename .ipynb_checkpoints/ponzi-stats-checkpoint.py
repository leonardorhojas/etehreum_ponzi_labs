...
for addr in ponzi:
    ...
    text = api.get_all_transactions()#api.get_ext_transactions()
    tint = api.get_all_transactions(internal=True)#api.get_int_transactions()
    ...
    for t in text:
        if (t['isError'] == '0'):
            ...
            count_text += 1
        if (verbose):
            s = 'from: ' + t['from'] + ',' + 'to: ' + t['to'] + ', eth: ' + str(
                float(t['value']) * 10 ** -18) + ', block: ' + t['blockNumber'] + ', isError : ' + t['isError']
            print(s)
    ...
    for t in tint:
        ...
        if (t['from'] == addr):  # internal transaction from contract to t['to']
            ...
            if (int(t['value']) > 0): users_text |= {t['from']}
        if (verbose):
            s = 'from: ' + t['from'] + ',' + 'to: ' + t['to'] + ', eth: ' + str(float(t['value']) * 10 ** -18) + ',' + \
                t['blockNumber'] + ', isError : ' + t['isError']
            print(s)
